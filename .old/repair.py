...
def get_rsrc(file:str, section_list:list)->list:
    pe = pefile.PE(file)
    for section in pe.sections:
        ...
        if section.Name == b".rsrc".ljust(8, b'\x00'):
            raw = section.PointerToRawData
            size = section.SizeOfRawData
            data = b"".join(open(file,'rb').readlines())[raw:raw+size]
            section_list.append({
                "name" : ".rsrc",
                "data" : data,
...
def set_section(file:str, section_list:list)->None:
    ...
    raw_start = utils.pe.align(header_end, 0x200)
    ...
    for i, section in enumerate(section_list):
        ...
        raw_pos = utils.pe.align(raw_pos + len(section["data"]), 0x200)
...
def base(file:str, section_rva:int, code_rva:int):
    m = MODIFY(file)
    m.pe.OPTIONAL_HEADER.AddressOfEntryPoint = code_rva
    m.pe.OPTIONAL_HEADER.BaseOfCode = section_rva
    m.set()
    m.close()